...
impl Bot {
    ...
    pub async fn next_event(&mut self) -> Option<Event> {
        ...
        match &event {
            ...
            Event::MovementParams {
                ...
                self.state.physics.apply_movement_params(
                    *walk_speed,
                    *jump_speed,
                    *gravity,
                );
    ...
    pub async fn physics_step(
        &mut self,
        dt: f32,
    ) -> Result<(), BotError> {
        let new_pos = self.state.physics.step(
            self.state.pos,
            dt,
            &self.state.blocks,
        );
    ...
    pub fn walk(
        ...
        self.state.physics.set_move_keys(
            yaw,
            forward,
            back,
            left,
...
impl Physics {
    pub fn step(
        ...
        {
            self.vel.x =
                self.wish_dir.x * self.walk_speed;

            self.vel.z =
                self.wish_dir.z * self.walk_speed;
        } else {
            self.vel.x = 0.0;
            self.vel.z = 0.0;
        }
        ...
        let min_x =
            ((next.x - hw) / BS).floor() as i32;

        let max_x =
            ((next.x + hw) / BS).floor() as i32;

        let min_z =
            ((next.z - hw) / BS).floor() as i32;
        ...
        if self.vel.y <= 0.0 {
            let from_by =
                (pos.y / BS).floor() as i32;

            let to_by =
                (next.y / BS).floor() as i32 - 1;
    ...
    pub fn set_move_keys(
        ...
        let wx =
            dx * rad.cos() - dz * rad.sin();

        let wz =
            dx * rad.sin() + dz * rad.cos();

        let len = (wx * wx + wz * wz).sqrt();

        self.wish_dir =
            Vector3::new(wx / len, 0.0, wz / len);
    }
